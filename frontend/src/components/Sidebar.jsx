...
const Sidebar = ({ activeChatId, setActiveChatId, recentChats, setRecentChats }) => {
  ...
  const handleIngest = async () => {
    ...
    const fauxEmbeddingTimer = setTimeout(() => {
      setIngestState(prev => ({ ...prev, step: 'embedding' }));
    }, 2500);
    ...
    try {
      ...
      clearTimeout(fauxEmbeddingTimer);

      if (response.ok) {
        if (data.alreadyExists) {
          setIngestState({ isOpen: false });
          setActiveChatId(data.namespace);
          setRepoUrl("");
          return;
        }

        
        setIngestState({ isOpen: true, step: 'success' });
        const repoName = data.repoName || repoUrl.split('/').pop() || "New Repo";
        ...
        setTimeout(() => {
          const newChat = { 
            ...
            isRepo: true,
            id: data.namespace // Uses the Pinecone namespace as the unique ID
          };
    ...
    <aside className="w-80 h-screen bg-[#0A0A0A] border-r border-white/5 flex flex-col z-20 shadow-2xl">
      ...
      <div className="px-4 mb-4">
        ...
        <div className="space-y-1">
          {folders.map((folder) => (
            <motion.div key={folder} whileHover={{ x: 4 }} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/[0.03] cursor-pointer text-zinc-400 hover:text-zinc-100 transition-all group">
          ...
          <AnimatePresence initial={false}>
            {recentChats.map((chat) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2.5">
                    <div className={`p-1.5 rounded-lg ${chat.isRepo ? "bg-green-500/20" : "bg-zinc-800"}`}>
                      {chat.isRepo ? <Github size={12} className="text-green-400" /> : <MessageSquare size={12} className="text-zinc-400" />}
                    </div>
                    <h4 className={`text-xs font-medium truncate w-32 ${activeChatId === chat.id ? "text-white" : "text-zinc-400"}`}>
                      {chat.title}
                    </h4>
                  </div>
                  {activeChatId === chat.id && (
                ...
                <p className="text-[10px] text-zinc-600 line-clamp-1 ml-9">{chat.desc}</p>
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
      ...
      <div className="p-6 border-t border-white/5 bg-[#0D0D0D]">
        <button 
          ...
          className="w-full bg-white text-black font-bold py-3 px-4 rounded-xl flex items-center justify-between hover:bg-green-400 transition-all active:scale-95 shadow-xl shadow-white/5"
        >
